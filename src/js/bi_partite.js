!function(){
	...
	var colors =["#3366CC", "#DC3912",  "#FF9900","#109618", "#990099", "#0099C6",
				...
				"#FFFF66","#6633CC","#336600","#663333","#990000","#CC3300"];
...
function bi_partite(year){
	...
	d3.json("json/"+year+"-Table1.json", function (error, data) {
		...
		var data = [ 
			{data:bP.partData(data.away,2), id:'Away_Games', header:["Venue","Teams", "Away Games"]},
			{data:bP.partData(data.home,2), id:'Home_Games', header:["Venue","Team", "Home Games"]}
		];
		console.log(data);
		bP.draw(data, svg);
...
function parse_bi_partite_data(data){
	...
	for (var i = 0; i < data.length; i++){
		...
		for (var j = 0; j < games.length; j++){
			console.log(games[j]);
			temp = add_to_away_data(games[j], temp);
			temp = add_to_home_data(games[j], temp);
		}
	}
	return temp;
}
